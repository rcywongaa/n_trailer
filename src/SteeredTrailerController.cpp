...
SteeredTrailerController::SteeredTrailerController() :
    ...
    steer_angle_input_idx(this->DeclareVectorInputPort(BasicVector<double>(1)).get_index()),
    joint_angle_input_idx(this->DeclareVectorInputPort(BasicVector<double>(1)).get_index()),
    ...
    steer_angle_output_idx(this->DeclareVectorOutputPort(BasicVector<double>(1), &SteeredTrailerController::calc_steer_angle).get_index())
{
    ...
    std::cout << "Controller steer_angle_input_idx = " << std::to_string(steer_angle_input_idx) << std::endl;
    std::cout << "Controller joint_angle_input_idx = " << std::to_string(joint_angle_input_idx) << std::endl;
...
const InputPortDescriptor<double>& SteeredTrailerController::steer_angle_input() const
{
    return System<double>::get_input_port(steer_angle_input_idx);
}

const InputPortDescriptor<double>& SteeredTrailerController::joint_angle_input() const
{
    return System<double>::get_input_port(joint_angle_input_idx);
}
...
{
    output->get_mutable_value()(0) = 0.5;
}
